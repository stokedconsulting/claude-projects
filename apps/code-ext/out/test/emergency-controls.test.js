...
const emergency_controls_1 = require("../emergency-controls");
...
suite('EmergencyControls Test Suite', () => {
    ...
    setup(() => {
        ...
        // Initialize managers
        lifecycleManager = new agent_lifecycle_1.AgentLifecycleManager(testWorkspaceRoot);
        ...
        emergencyControls = new emergency_controls_1.EmergencyControls(lifecycleManager, sessionManager, queueManager, testWorkspaceRoot);
    });
    teardown(() => {
     ...
     */
    async function createTestSession(agentId, status = 'idle') {
     ...
     */
    async function createTestClaim(agentId, projectNumber, issueNumber) {
        await queueManager.claimProject(projectNumber, issueNumber, `agent-${agentId}`);
    }
    /**
     * Helper: Create stale claim (> 8 hours old)
     */
    async function createStaleClaim(agentId, projectNumber, issueNumber) {
...
//# sourceMappingURL=emergency-controls.test.js.map