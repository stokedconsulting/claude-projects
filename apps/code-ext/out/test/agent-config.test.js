...
suite('Agent Configuration Test Suite', () => {
    ...
    test('AC-1.1.a: Valid configuration loads successfully within 2 seconds', async () => {
        ...
        const agentConfig = (0, agent_config_1.getAgentConfig)();
    ...
    test('AC-1.1.d: Invalid category names are filtered out with warning', async () => {
        ...
        assert.ok(!agentConfig.enabledCategories.includes('InvalidCategory1'), 'Should not include InvalidCategory1');
    });
    test('AC-1.1.d: All invalid categories defaults to all valid categories', async () => {
        ...
        const agentConfig = (0, agent_config_1.getAgentConfig)();
        assert.strictEqual(agentConfig.enabledCategories.length, agent_config_1.VALID_AGENT_CATEGORIES.length, 'Should enable all valid categories when all are invalid');
    });
    test('Valid categories constant has 21 categories', () => {
        assert.strictEqual(agent_config_1.VALID_AGENT_CATEGORIES.length, 21, 'Should have exactly 21 valid categories');
    });
    test('isCategoryEnabled returns true for enabled category', async () => {
        ...
        assert.ok((0, agent_config_1.isCategoryEnabled)('Security'), 'Security should be enabled');
        assert.ok((0, agent_config_1.isCategoryEnabled)('Testing'), 'Testing should be enabled');
        assert.ok(!(0, agent_config_1.isCategoryEnabled)('Documentation'), 'Documentation should not be enabled');
    });
    test('getDisabledCategories returns correct disabled categories', async () => {
        const config = vscode.workspace.getConfiguration('claudeProjects.agents');
        await config.update('enabledCategories', ['Security', 'Testing'], vscode.ConfigurationTarget.Global);
        const disabled = (0, agent_config_1.getDisabledCategories)();
        assert.strictEqual(disabled.length, agent_config_1.VALID_AGENT_CATEGORIES.length - 2, 'Should have 19 disabled categories');
    ...
    test('Configuration can be read multiple times without errors', async () => {
        const config = vscode.workspace.getConfiguration('claudeProjects.agents');
        await config.update('maxConcurrent', 3, vscode.ConfigurationTarget.Global);
        const config1 = (0, agent_config_1.getAgentConfig)();
        const config2 = (0, agent_config_1.getAgentConfig)();
        const config3 = (0, agent_config_1.getAgentConfig)();
