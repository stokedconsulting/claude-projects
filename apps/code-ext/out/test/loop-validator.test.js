...
suite('LoopValidator Test Suite', () => {
    ...
    setup(async () => {
        ...
        const mockGitHubApi = {};
        projectQueueManager = new project_queue_manager_1.ProjectQueueManager(tempDir, mockGitHubApi);
        reviewQueueManager = new review_queue_manager_1.ReviewQueueManager(tempDir);
        // Initialize LoopValidator
        loopValidator = new loop_validator_1.LoopValidator(tempDir, sessionManager, projectQueueManager, reviewQueueManager);
    });
    teardown(async () => {
    ...
    test('AC-4.5.d: Full cycle time tracked and under 4 hours', async () => {
        ...
        const states = ['idle', 'working', 'idle', 'reviewing', 'idle', 'ideating', 'idle', 'working'];
    ...
    test('AC-4.5.e: Category coverage tracked', async () => {
        ...
        assert.ok(report.coveragePercent >= 0 && report.coveragePercent <= 100, 'Coverage percent should be between 0 and 100');
    });
    /**
    ...
    test('Validate loop health detects stuck agents', async () => {
        ...
        assert.ok(health.recommendations.some(r => r.includes('stuck')), 'Recommendations should mention stuck agents');
    });
    /**
    ...
    test('Validate loop health recommends prioritizing ideation when queue low', async () => {
        ...
        await projectQueueManager.claimProject(79, 1, 'agent-1');
        const health = await loopValidator.validateLoopHealth();
        assert.ok(health.recommendations.some(r => r.includes('Prioritize ideation')), 'Should recommend prioritizing ideation');
    });
    /**
    ...
    test('Validate loop health recommends pausing ideation when queue high', async () => {
        ...
        const health = await loopValidator.validateLoopHealth();
        assert.ok(health.recommendations.some(r => r.includes('Pause ideation')), 'Should recommend pausing ideation');
    });
    /**
...
//# sourceMappingURL=loop-validator.test.js.map