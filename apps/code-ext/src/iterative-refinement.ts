...
import { ReviewQueueManager, ReviewItem } from './review-queue-manager';

/**
 * Review feedback structure containing details about rejection
 */
...
/**
 * Review history entry for a single review cycle
 */
...
/**
 * Review cycle state for an issue
 */
...
/**
 * Review cycles storage structure
 * Maps issueNumber to its cycle state
 */
interface ReviewCyclesStorage {
    [issueNumber: number]: ReviewCycleState;
}
...
export class IterativeRefinementManager {
    ...
    private readonly CYCLES_FILE = 'review-cycles.json';
    ...
    constructor(workspaceRoot: string) {
        ...
        this.reviewQueueManager = new ReviewQueueManager(workspaceRoot);
    }
     ...
     */
    private getCyclesFilePath(): string {
     ...
     */
    private readCyclesFile(): ReviewCyclesStorage {
        ...
        try {
            ...
            return parsed as ReviewCyclesStorage;
        } catch (error) {
     ...
     */
    private writeCyclesFileAtomic(storage: ReviewCyclesStorage): void {
     ...
     */
    public async getReviewCycleCount(issueNumber: number): Promise<number> {
     ...
     */
    public async incrementReviewCycle(issueNumber: number, projectNumber: number): Promise<number> {
        const storage = this.readCyclesFile();
     ...
     */
    private async addHistoryEntry(issueNumber: number, entry: ReviewHistoryEntry): Promise<void> {
        const storage = this.readCyclesFile();
        const state = storage[issueNumber];
     ...
     */
    public async getReviewHistory(issueNumber: number): Promise<ReviewHistoryEntry[]> {
        const storage = this.readCyclesFile();
        const state = storage[issueNumber];
     ...
     */
    public async shouldEscalate(issueNumber: number): Promise<boolean> {
     ...
     */
    public formatFeedbackComment(feedback: ReviewFeedback, cycleNumber: number): string {
        const lines: string[] = [];
     ...
     * @param feedback - Review feedback data
     */
    public async handleReviewRejection(reviewId: string, feedback: ReviewFeedback): Promise<void> {
     ...
     */
    private async writeReviewFeedbackFile(issueNumber: number, comment: string): Promise<void> {
        this.ensureSessionsDirectory();

        const feedbackPath = path.join(
            this.getSessionsDirectory(),
            `issue-${issueNumber}-feedback.md`
        );
     ...
     */
    private async reenqueueIssue(review: ReviewItem): Promise<void> {
     ...
     */
    public async escalateToUser(issueNumber: number, reviewHistory: ReviewHistoryEntry[]): Promise<void> {
        ...
        const escalationPath = path.join(
            this.getSessionsDirectory(),
            `issue-${issueNumber}-escalation.md`
        );
     ...
     */
    private generateEscalationSummary(issueNumber: number, reviewHistory: ReviewHistoryEntry[]): string {
        const lines: string[] = [];
     ...
     */
    public async getLatestReviewFeedback(issueNumber: number): Promise<ReviewFeedback | null> {
     ...
     * @param issueNumber - GitHub issue number
     */
    public async clearReviewCycles(issueNumber: number): Promise<void> {
        const storage = this.readCyclesFile();
     ...
     */
    public async getActiveRefinementIssues(): Promise<number[]> {
        const storage = this.readCyclesFile();
 ...
 */
let refinementManagerInstance: IterativeRefinementManager | null = null;

/**
 * Initialize the iterative refinement manager with workspace root
 * Should be called during extension activation
 */
export function initializeIterativeRefinement(workspaceRoot: string): IterativeRefinementManager {
 ...
 */
export function getIterativeRefinementManager(): IterativeRefinementManager {
 ...
 */
export function cleanupIterativeRefinement(): void {
