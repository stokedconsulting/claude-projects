...
import { AgentExecutor, ExecutionResult, ExecutionStatus } from '../agent-executor';
...
suite('AgentExecutor Test Suite', () => {
    ...
    let mockGithubApi: GitHubAPI;
    let executor: AgentExecutor;
    const PROJECT_ID = 'test-project-id';
    const PROJECT_NUMBER = 79;

    // Helper to create mock project items
    function createMockProjectItem(issueNumber: number, status: string = 'todo'): ProjectItem {
        return {
            id: `item-${issueNumber}`,
    ...
    setup(async () => {
        ...
        mockGithubApi = {} as GitHubAPI; // Mock API - we'll override methods
        ...
        // Initialize executor
        executor = new AgentExecutor(testWorkspaceRoot, mockGithubApi, PROJECT_ID);
    });
    ...
    test('AC-2.2.a: Agent status transitions to working after claiming issue', async () => {
        ...
        mockGithubApi.getProjectItems = async (projectId: string) => {
    ...
    test('AC-2.2.d: Agent returns to idle and tasks completed increments', async () => {
        ...
        assert.strictEqual(
            finalSession?.tasksCompleted,
            initialTasksCompleted + 1,
            'Tasks completed should increment by 1'
        );
    ...
    test('AC-2.2.f: Execution respects 8-hour timeout', async () => {
        ...
        const timeoutValue = (executor as any).EXECUTION_TIMEOUT_MS;
    ...
    test('isAgentExecuting returns correct execution status', async () => {
        ...
        assert.strictEqual(
            executor.isAgentExecuting(agentId),
            false,
        ...
        assert.strictEqual(
            executor.isAgentExecuting(agentId),
            false,
            'Agent should not be executing when no work available'
        );
    ...
    test('Execution loop skips iteration if agent is busy', async () => {
        ...
        const finalSession = await sessionManager.readAgentSession(agentId);
        assert.strictEqual(
            finalSession?.tasksCompleted ?? 0,
            1,
            'Should have completed exactly 1 task (no double execution)'
        );
    });
});
