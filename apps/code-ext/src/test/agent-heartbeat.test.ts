...
import { AgentHeartbeatManager, AgentHealthStatus, HeartbeatPayload, HealthStatusResult } from '../agent-heartbeat';
...
suite('AgentHeartbeatManager Test Suite', () => {
    ...
    test('AC-1.3.a: Heartbeat is sent every 30 seconds with current status', async function() {
        ...
        assert.ok(
            uniqueHeartbeats.size >= 2,
            `Should have at least 2 unique heartbeats, got ${uniqueHeartbeats.size}`
        );
    ...
    test('AC-1.3.b: Health status transitions based on heartbeat timing', async function() {
        ...
        assert.ok(health.timeSinceLastHeartbeat !== null);
        assert.ok(
            health.timeSinceLastHeartbeat >= 60000 && health.timeSinceLastHeartbeat < 120000,
            'Time since last heartbeat should be 60-120s'
        );
    ...
    test('stopHeartbeat stops periodic heartbeats', async function() {
        ...
        assert.strictEqual(
            firstHeartbeat,
            secondHeartbeat,
            'Heartbeat should not update after stopping'
        );
    });
    ...
    test('Heartbeat continues after network error (simulated)', async function() {
        ...
        assert.notStrictEqual(
            firstHeartbeat,
            secondHeartbeat,
            'Heartbeat should continue even with network errors'
        );
