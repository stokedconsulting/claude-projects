...
import {
    IterativeRefinementManager,
    ReviewFeedback,
    ReviewHistoryEntry,
    initializeIterativeRefinement,
    getIterativeRefinementManager,
    cleanupIterativeRefinement
...
suite('Iterative Refinement Manager Tests', () => {
    ...
    let refinementManager: IterativeRefinementManager;
    ...
    setup(() => {
        ...
        refinementManager = new IterativeRefinementManager(testWorkspaceRoot);
        reviewQueueManager = new ReviewQueueManager(testWorkspaceRoot);
    });
    ...
    suite('State Persistence - AC-3.5.f', () => {
        test('should persist cycle count across manager instances', async () => {
            ...
            const manager1 = new IterativeRefinementManager(testWorkspaceRoot);
            await manager1.incrementReviewCycle(123, 79);
            await manager1.incrementReviewCycle(123, 79);

            // Create second manager and verify count
            const manager2 = new IterativeRefinementManager(testWorkspaceRoot);
    ...
    suite('Singleton Instance Management', () => {
        test('should initialize singleton instance', () => {
            cleanupIterativeRefinement();
            const instance = initializeIterativeRefinement(testWorkspaceRoot);
            assert.ok(instance, 'Should return instance');

            const retrieved = getIterativeRefinementManager();
        ...
        test('should throw error when accessing uninitialized singleton', () => {
            ...
            assert.throws(
                () => getIterativeRefinementManager(),
                /not initialized/i,
                'Should throw error for uninitialized instance'
            );
        });

        test('should cleanup singleton instance', () => {
            initializeIterativeRefinement(testWorkspaceRoot);
            cleanupIterativeRefinement();
