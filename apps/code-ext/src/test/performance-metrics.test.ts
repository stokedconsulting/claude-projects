...
import { PerformanceMetrics, TaskCompletionData, AgentMetrics } from '../performance-metrics';
...
suite('Performance Metrics Tests', () => {
    ...
    setup(() => {
        ...
        sessionManager = new AgentSessionManager(testWorkspace);
        performanceMetrics = new PerformanceMetrics(testWorkspace, sessionManager);
    });
    ...
    test('AC-5.4.c: When metrics card is expanded → all tracked metrics are displayed', async () => {
        ...
        const now = Date.now();
        const completions: TaskCompletionData[] = [
            {
                projectNumber: 1,
    ...
    test('Metrics persist across instances', async () => {
        ...
        const newMetrics = new PerformanceMetrics(testWorkspace, sessionManager);
